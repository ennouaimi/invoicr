...
import { money, ReceiptPayload, receiptHtml, totals, validateReceipt } from "../../../../lib/receipt";
...
export async function POST(request:Request){
 ...
 const error=validateReceipt(body);if(error)return Response.json({error},{status:422,headers:cors});
 const format=new URL(request.url).searchParams.get("format")??"pdf";
 if(format==="html")return new Response(receiptHtml(body),{headers:{...cors,"Content-Type":"text/html; charset=utf-8"}});
 if(format==="json")return Response.json({receipt:body,totals:totals(body)},{headers:cors});
 ...
 text(`Receipt: ${body.receiptNumber??"RECEIPT"}`);text(`Date: ${body.date??new Date().toISOString().slice(0,10)}`);text(`Payment: ${body.payment?.method??"-"}${body.payment?.last4?" ****"+body.payment.last4:""}`);y-=8;
 ...
 const bytes=await pdf.save();const filename=(body.receiptNumber??"receipt").replace(/[^a-z0-9_-]/gi,"-");
