...
export default function Docs(){
 const curl=`curl -X POST "https://receipt-generator-topaz-three.vercel.app/api/v1/invoices?format=pdf" \\\n  -H "Content-Type: application/json" \\\n  -d '${payload.replace(/\n/g," ")}' \\\n  --output invoice.pdf`;
 const js=`const response = await fetch("/api/v1/invoices?format=pdf", {\n  method: "POST",\n  headers: { "Content-Type": "application/json" },\n  body: JSON.stringify(invoice)\n});\nconst pdf = await response.blob();`;
 const py=`import requests\n\nr = requests.post(\n    "https://receipt-generator-topaz-three.vercel.app/api/v1/invoices?format=pdf",\n    json=invoice\n)\nopen("invoice.pdf", "wb").write(r.content)`;
